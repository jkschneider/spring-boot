...
class HealthMetricsExportAutoConfigurationTests {
	...
	void autoConfiguresConfigAndMeterRegistry() {
		this.contextRunner.withUserConfiguration(BaseConfiguration.class)
				.run((context) -> assertThat(context).hasSingleBean(HealthMeterRegistry.class)
						.hasSingleBean(HealthConfig.class));
	}
	...
	void autoConfiguresHealthIndicators() {
		this.contextRunner.withUserConfiguration(BaseConfiguration.class)
				...
				.run((context) -> assertThat(context)
						.hasBean("apiErrorRatioApiCustomer")
						.hasBean("apiErrorRatioAdmin")
						.hasBean("jvmGcLoad")
						.hasBean("jvmPoolMemory")
						.hasBean("jvmTotalMemory"));
	}
	...
	void allowsCustomRegistryToBeUsed() {
		this.contextRunner.withUserConfiguration(CustomRegistryConfiguration.class)
				.run((context) -> assertThat(context).hasSingleBean(HealthMeterRegistry.class)
						.hasBean("customRegistry").hasSingleBean(HealthConfig.class));
	}
	...
	static class CustomRegistryConfiguration {
		...
		HealthMeterRegistry customRegistry(HealthConfig config, Clock clock) {
			return HealthMeterRegistry.builder(config)
					.clock(clock)
					.build();
		}
